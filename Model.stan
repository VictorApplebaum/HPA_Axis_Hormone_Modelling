...
model {
  ...
  for (v in 1:N_vars){
    pulse_1_length_v[v] ~ normal(0,5);
    pulse_1_Cortisol_effect_v[v] ~ normal(0,0.1);
    pulse_1_CCS_effect_v[v] ~ normal(0,0.1);
    pulse_2_length_v[v] ~ normal(0,5);
    pulse_2_init_v[v] ~ normal(0,5);
    pulse_2_Cortisol_effect_v[v] ~ normal(0,0.1);
    pulse_2_CCS_effect_v[v] ~ normal(0,0.1);
    IIBetaHSD2_v[v] ~ normal(0,1);
    VBetaReductase_v[v] ~ normal(0,0.1);
    VAlphaReductase_v[v] ~ normal(0,0.1);
    XVIIIHydroxylase_v[v] ~ normal(0,0.1);
    XIBetaHydroxylase_v[v] ~ normal(0,0.1);
    IIBetaHSD1_v[v] ~ normal(0,0.1);

      for (hormone in 1:N_hormones){
        decay_v[hormone,v]~ normal(0,1);
        init_v[hormone,v]~ normal(0,0.1);
        sigma_v[hormone,v]~ normal(0,0.1);
      }
  }
